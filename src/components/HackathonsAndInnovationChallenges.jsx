...
// LOCAL DATA (renamed from your inline list)
...
// helper derived data
const highlightDates = hackathonsData.map((b) => new Date(b.date));
...
const HackathonsAndInnovationChallenges = () => {
  ...
  const filteredHackathons = useMemo(() => {
    const now = new Date();
    return hackathonsData.filter((b) => {
      const eventDate = new Date(b.date);
      // try to estimate start time by parsing first part of time (best-effort)
  ...
  const paginatedHackathons = filteredHackathons.slice(0, visibleCount);

  const selectedHackathons = useMemo(() => {
  ...
  const handleEnrollClick = (item) => {
    ...
    // navigate to enroll page (App route: /enroll-now)
...
export default HackathonsAndInnovationChallenges;
